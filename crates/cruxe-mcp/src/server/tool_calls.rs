...
fn tool_compatibility_error(params: ToolCompatibilityParams<'_>) -> JsonRpcResponse {
    ...
    let remediation = match schema_status {
        SchemaStatus::NotIndexed => "cruxe index",
        SchemaStatus::ReindexRequired | SchemaStatus::CorruptManifest => "cruxe index --force",
        SchemaStatus::Compatible => "cruxe index",
    };
    let message = match schema_status {
        SchemaStatus::NotIndexed => "No index available. Run `cruxe index`.",
        SchemaStatus::ReindexRequired | SchemaStatus::CorruptManifest => {
            "Index is incompatible. Run `cruxe index --force`."
        }
...
mod tests {
    ...
    fn ranking_payload_basic_uses_compact_fields() {
        ...
        let payload =
            ranking_reasons_payload(reasons, cruxe_core::types::RankingExplainLevel::Basic)
                .unwrap();
