...
use codecompass_indexer::parser;
use codecompass_state::{db, project};
use std::path::Path;
        ...
        match db::open_connection(&db_path) {
            Ok(conn) => {
                ...
                match project::get_by_root(&conn, &repo_root_str) {
                    ...
                    Ok(None) => {
                        println!("MISSING - run `codecompass init`");
                        all_ok = false;
                    }
                    Err(e) => {
                        println!("FAIL: {}", e);
    ...
    for index_name in &["symbols", "snippets", "files"] {
        ...
        if index_dir.exists() {
            match tantivy::Index::open_in_dir(&index_dir) {
                Ok(_) => println!("OK"),
                Err(e) => {
                    println!("FAIL: {} - run `codecompass index --force`", e);
                    all_ok = false;
                }
            }
        } else {
